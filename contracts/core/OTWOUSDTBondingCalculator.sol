...
contract OTWOUSDTBondingCalculator is IBondingCalculator {
    ...
    function getKValue( address _pair ) public view returns( uint k_ ) {
        ...
        k_ = reserve0.mul(reserve1).div( 10 ** decimals ).div(100);
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == OTWO ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( OTWO ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}